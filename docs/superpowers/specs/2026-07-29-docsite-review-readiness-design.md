...
2. **`CLN_CCS` maturity annotation is now stale.** `docs/workflows/hod/cln/index.md` currently states `CLN_CCS` is "**not yet specified by the SVCv4 Working Group**" — true when PR #18 was authored (2026-06-11), but Supplementary Material 4 (Clinical Observations), now in hand, fully specifies `CLN_CCS` (odds-ratio/CI-based case-control evidence). The correct annotation is now the *other* category this repo's own convention distinguishes: "**specified by the SVCv4 Standards but not yet modeled here**" — the same bucket as POP/LOC/PFD, not a Working-Group gap.
...
1. **Model completeness gaps surfaced by Supplementary Material 4:** add `CLN_CCS` to `Workflow` (and corresponding `Case`/applicability-matrix entries for OR/CI); add a gnomAD co-occurrence-likelihood-bucket field and a "non-genetic etiology excluded" field to `Case`/`CaseTesting`.
