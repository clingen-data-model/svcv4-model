...
| 3 | [Population Database Frequency](https://docs.google.com/document/d/1XON2eq4HSM-guWlqitEnb8PghY0quNbA7_1WmmxvLj8/edit) | `POP_FRQ`, `POP_HMZ` | Not yet modeled — see [Population (POP)](../workflows/hod/pop.md). Introduces the Cohort Allele Frequency and Disease Allele Frequency Threshold concepts — see [Core concepts](concepts.md) |
| 4 | [Clinical Observations](https://docs.google.com/document/d/17XnPmgTpzgQ8hfzOZNCcXIBwnNwbVkP0KIv3Dw5Bz_M/edit) | `CLN_CCS`/`CLN_AFF`/`CLN_DNV`/`CLN_ALT`/`CLN_UAF` | **Modeled** for AFF/DNV/ALT/UAF via the Case model — see [Clinical Observations (CLN)](../workflows/hod/cln/index.md). `CLN_CCS` is a special case: SVCv4 provides scoring guidance for it but not decomposed evidence concepts, so it's out of scope for this Classification Model until a future SVCv4 version adds those (see the note on the CLN page) — not a simple backlog item |
...
| 18 | [Molecular Mechanism and Exon Relevance](https://docs.google.com/document/d/1BLnsgxLY0TibwylFWz0SeGGeCusWwSokrgU4qsmBiaw/edit) | (shared sub-module) | Not yet modeled — but the **gene-disease validity** gate this supplement depends on (mechanism scoring applies only at Moderate+ validity) is now captured on `WorkflowParameters` (see [Core concepts](concepts.md)); the mechanism multiplier itself arrives with PFD |
| 19 | [Informative Variants](https://docs.google.com/document/d/1hNfdtdvDT4dob9oDBrL_UzVV_MYiWnwERfli76EAbyQ/edit) | (shared sub-module) | Not yet modeled |
| 20 | [Functional Assays](https://docs.google.com/document/d/1X68otBl4YvdXlP1bOD83JO4kIod0Ol5BoLB4CLxqijA/edit) | (shared sub-module) | Not yet modeled |
