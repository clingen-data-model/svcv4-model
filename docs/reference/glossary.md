...
| **VBC → `CanonicalAllele` / `Allele`** | The Variant Being Considered — the germline variant under evaluation — is **represented in the model as a GA4GH `CanonicalAllele` (or `Allele`)**. "The variant," "variation," and "VBC" are synonyms for it; it is the Proposition subject (`subjectVariant`). *Additional variant* and *compound-het variant* are other, distinctly-named variants — not the VBC. |
