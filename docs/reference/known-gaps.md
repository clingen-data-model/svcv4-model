...
| LOC_SEG Figure-2 entry gate not captured | Case model | SM 5 Figure 2 gates co-segregation scoring on **>1 locus** *and* a **phenocopy rate very low/zero**, but the model has no field for either. `reference_score_loc_seg` **assumes the gate is satisfied** and flags it in provenance. Two further reference-scorer choices are flagged for confirmation: unaffected non-AR co-segregants are counted only at near-100% penetrance (the AR `+0.4` is not penetrance-gated), and a **semidominant** non-segregation is **not** given the `−4.0` flip (only AD / AR-homozygous / X-linked are, per the SM 5 Figure 2 note). |
...
| Item | Source | Notes |
|---|---|---|
| SM 6 missense-splice blue/violet parent caps | SM 6 | SM 6's missense-splice **blue (`UNCERTAIN`) → `−8..0`** and **violet (`UNLIKELY`) → `−8..+8`** parent caps are inverted vs Canonical (SM 11) / Intronic (SM 12), and read backwards (uncertain capped at 0; unlikely allowed positive). Encoded faithfully to SM 6 + `missense.md`. This is a good candidate to raise with the working group — same category as the other image-only / ambiguous-source items on this page. |
...
| SM 3 POP_HMZ Autosomal Dominant weight | SM 3 | Prose (L93) says −0.5 pts per homozygous occurrence for AD *or* AR, but **Table 7** assigns Autosomal Dominant **−1.0** (AR/semidominant/X-linked −0.5). The reference scorer follows Table 7 (the explicit point-value table). A good candidate to raise with the working group — same category as the other image-only / ambiguous-source items on this page. |
| SM 5 LOC_PHE Figure-1 `+2.0` band + boundaries | SM 5 | SM 5's text anchors the phenotype-specificity bands `<33→0`, `33-50→+1`, `68-81→+3`, `>82→+4`, but gives **no explicit anchor for the `+2.0` band** (the reference scorer uses `50<pct<68`) and leaves the `(81,82)` sliver undefined (folded **down** to `+3` conservatively; `≥82→+4` slightly over-awards vs the "83%" worked examples). A candidate to confirm with the working group — same category as the other image-only / ambiguous-source items on this page. |
