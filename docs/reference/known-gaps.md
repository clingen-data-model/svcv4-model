...
| Full PFD modeling | PFD | Two shared sub-modules are now modeled (inputs only): Molecular Mechanism & Exon Relevance (SM 18, `MechanismExonRelevanceEvidence`) and Informative Variants (SM 19, `InformativeVariantsEvidence`). What remains: Functional Assays (SM 20) and Critical Amino Acids (SM 7); the PRD/FXN/INF scaffold + parent codes (NUL/CDS/SPL/MIS); the per-variant-type workflows; and the multiplier/scoring computation. See [Predictive & Functional Data](../workflows/pfd/index.md). |
