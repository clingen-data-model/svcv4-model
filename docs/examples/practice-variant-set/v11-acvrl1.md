...
=== "Narrative"
    ...
    - **Affected with an alternate cause** (`CLN_ALTG`, score -1.0) — Alternate cause — gene (illustrative).
    - **Single-amino-acid change (MIS)** (`MIS`, score 0.0) — Predictive missense evidence (PFD, illustrative).
      - `MIS_PRD` (score 0.0) — In-silico predictive missense assessment.
        - `MIS_PRD_INIT_REVEL` (score 0.0) · *provisional* — REVEL 0.499 is intermediate; no other reported variants in this codon.
...
=== "Semi-structured"
    ...
    Statement
      proposition:
        subject (VBC): ACVRL1 c.88C>T (p.Pro30Ser)
