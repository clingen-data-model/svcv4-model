...
- `Statement` — VA-Spec `Statement` profiled for SVCv4. Carries a `Proposition`, a `score`, `strength`, `direction`, `outcome`, `method` (reference — identifies the **applied SVCv4 specification version**: baseline or a specific VCEP specialisation), `contribution`, and `evidence_lines: list[EvidenceLine]`.
...
- `EvidenceLine` — VA-Spec `EvidenceLine` profiled for SVCv4. Captures the result of invoking a CSpec method/rule on a set of evidence items: `score`, optional `outcome`, the evidence items used (`evidence: list[EvidenceItem]`), the invoked method (`method` reference) plus its `code` (evidence-code / method-code reference), optional `strength` and `direction`, and `contribution`. Any CSpec method or rule that produces a score lands here.
